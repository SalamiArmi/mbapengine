...
void XInputManager::Update()
{
	...
	for(int i = 0; i<4;i++)
	{
		...
		if(dwResult == ERROR_SUCCESS)
		{
			...
			{
				Pads[i].LeftStick.X = state[i].Gamepad.sThumbLX / 32768.0f;
				Pads[i].LeftStick.Y = state[i].Gamepad.sThumbLY / 32768.0f;
			}
			...
			{
				Pads[i].RightStick.X = state[i].Gamepad.sThumbRX / 32768.0f;
				Pads[i].RightStick.Y = state[i].Gamepad.sThumbRY / 32768.0f;
			}
...
const Xbox360State &XInputManager::GetState(const int pad) const
{
...
void XInputManager::SetVibration(const int pad, const float left, const float right) const
{
	if(pad > 3)
		return;
