...
namespace Argon
{
	D3D10Driver::D3D10Driver(D3D10Device* Device, uint AdapterIndex, IDXGIAdapter* Adapter)
		: m_Device(Device),
	...
	String D3D10Driver::GetName()
	{
		DXGI_ADAPTER_DESC AdapterDesc;
		m_Adapter->GetDesc(&AdapterDesc); //Get the adapter description
	...
	String D3D10Driver::GetDescription()
	{
		return GetName();
	}

	IVideoMode* D3D10Driver::GetVideoMode(uint Index)
	{
		return m_VideoModes.At(Index);
	}

	uint D3D10Driver::GetVideoModeCount()
	{
		return m_VideoModes.Size();
	}

	void D3D10Driver::GetModeList()
	{
		HRESULT hr = S_OK;
		IDXGIOutput* Output;
		...
		{	
			...
			else //Worked
			{
				...
				*/
				...
				{
					D3D10VideoMode* VideoMode = new D3D10VideoMode(OutputDesc, Desc[ModeIndex]); //can be used for multi-moniter support
	...
	IDXGIAdapter* D3D10Driver::GetAdapter()
	{
		return m_Adapter;
	}
