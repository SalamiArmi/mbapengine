...
namespace Argon
{
	D3D10VideoMode::D3D10VideoMode(DXGI_OUTPUT_DESC OutputDesc, DXGI_MODE_DESC ModeDesc)
	...
	String D3D10VideoMode::GetDescription()
	{
	...
	uint D3D10VideoMode::GetColorDepth()
	{
		uint ColorDepth = 16;
	...
	uint D3D10VideoMode::GetWidth()
	{
		return m_ModeDesc.Width;
	}

	uint D3D10VideoMode::GetHeight()
	{
		return m_ModeDesc.Height;
	}

	Vector2T<uint> D3D10VideoMode::GetRefereshRate()
	{
		return Vector2T<uint>(m_ModeDesc.RefreshRate.Numerator, m_ModeDesc.RefreshRate.Denominator);
	}

	DXGI_MODE_DESC D3D10VideoMode::GetMode()
	{
		return m_ModeDesc;
	}
} //Namespace